...
const sendcooldowncommand = new Map();
...
module.exports = {
    ...
    .setName('send')
    .setDescription('Send messages that you type in code block')
    ...
    async execute(interaction) {
        ...
        if(sendcooldowncommand.has(interaction.user.id)) {
            const embed = new Discord.EmbedBuilder()
        ...
        if(!chtarget) {
            dbl.hasVoted(interaction.user.id).then(voted => {
                if(voted) {
                    ...
                    sendcooldowncommand.set(interaction.user.id);
                    console.log(`[SENDCOOLDOWN] ADD ${interaction.user.tag}(${interaction.user.id})`);
                    setTimeout(() => {
                        sendcooldowncommand.delete(interaction.user.id)
                        console.log(`[SENDCOOLDOWN] REMOVE ${interaction.user.tag}(${interaction.user.id})`)
                    },60000);
                };
                if(!voted) {
                    const embed = new Discord.EmbedBuilder()
